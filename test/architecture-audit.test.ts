...
// OIDC and execution-file tests are regressions; others characterize defects.
...
test("an early failure cannot publish a previous invocation's execution file", async () => {
  const env = { ...process.env };
  ...
  try {
    ...
    // Invocation B uses the same RUNNER_TEMP but fails before any SDK output.
    initializeExecutionFile();
    expect(getExecutionFilePath()).not.toBe(previous!);
    expect(setExecutionFileOutputIfPresent()).toBeUndefined();
    expect(output).not.toHaveBeenCalled();
    expect(existsSync(previous!)).toBe(true);

    // Recovery still publishes a partial transcript from this invocation.
    const current = await writeExecutionFile([
      { type: "system", session_id: "current-run" },
    ]);
    expect(current).toBeDefined();
    expect(setExecutionFileOutputIfPresent()).toBe(current!);
    expect(output).toHaveBeenCalledWith("execution_file", current!);
  } finally {
    output.mockRestore();
    process.env = env;
    rmSync(dir, { recursive: true, force: true });
