...
describe("pull_request_target event support", () => {
  ...
  describe("prompt generation for pull_request_target", () => {
    test("should generate correct prompt for pull_request_target event", () => {
      ...
      expect(prompt).toContain("<trigger_context>pull request opened</trigger_context>");

      // Should contain PR-specific information
      expect(prompt).toContain("- src/feature.ts (MODIFIED) +20/-2 SHA: abc123");
      expect(prompt).toContain("- tests/feature.test.ts (ADDED) +5/-1 SHA: abc123");
    ...
    test("should handle pull_request_target with commit signing disabled", () => {
      ...
      expect(prompt).toContain("Always push to the existing branch when triggered on a PR");
    ...
    test("should treat pull_request_target same as pull_request in prompt generation", () => {
      ...
      const pullRequestPrompt = generatePrompt(pullRequestContext, mockGitHubData, false, mockTagMode);
      const pullRequestTargetPrompt = generatePrompt(pullRequestTargetContext, mockGitHubData, false, mockTagMode);

      // Both should have the same event type and structure
      expect(pullRequestPrompt).toContain("<event_type>PULL_REQUEST</event_type>");
      expect(pullRequestTargetPrompt).toContain("<event_type>PULL_REQUEST</event_type>");

      expect(pullRequestPrompt).toContain("<trigger_context>pull request opened</trigger_context>");
      expect(pullRequestTargetPrompt).toContain("<trigger_context>pull request opened</trigger_context>");

      // Both should contain PR-specific instructions
      expect(pullRequestPrompt).toContain("Always push to the existing branch when triggered on a PR");
      expect(pullRequestTargetPrompt).toContain("Always push to the existing branch when triggered on a PR");
    });
    ...
    test("should handle pull_request_target in agent mode with custom prompt", () => {
      ...
      const prompt = generatePrompt(envVars, mockGitHubData, false, mockAgentMode);

      expect(prompt).toBe("Review this pull_request_target PR for security issues");
    });
  ...
  describe("security considerations for pull_request_target", () => {
    test("should maintain same prompt structure regardless of event source", () => {
      ...
      const internalPrompt = generatePrompt(internalPR, mockGitHubData, false, mockTagMode);
      const externalPrompt = generatePrompt(externalPR, mockGitHubData, false, mockTagMode);

      // Should have same tool access patterns
      expect(internalPrompt.includes("mcp__github_comment__update_claude_comment"))
        .toBe(externalPrompt.includes("mcp__github_comment__update_claude_comment"));

      // Should have same branch handling instructions
      expect(internalPrompt.includes("Always push to the existing branch when triggered on a PR"))
        .toBe(externalPrompt.includes("Always push to the existing branch when triggered on a PR"));
    });
  });
});