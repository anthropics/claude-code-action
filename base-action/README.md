      ...
      - name: Extract and Comment PR Review
        ...
        with:
          ...
          script: |
            try {
              const fs = require('fs')
              const executionFile = '${{ steps.code-review-prompt.outputs.execution_file }}';
              const executionString = fs.readFileSync(executionFile, 'utf8')
              const executionLog = JSON.parse(executionString)
              ...
              for (let i = executionLog.length - 1; i >= 0; i--) {
               ...
               // Check for the result field (final output)
               ...
               if (entry.role === 'assistant' && entry.content) {
                 ...
                 review = entry.text;
                 break;
               } else if (entry.sender === 'assistant' && entry.message) {
                 review = entry.message;
                 break;
               }
              }
              
              if (!review || review.trim() === '') {
               review = "Claude Code review completed but no content was extracted. Check the execution logs for details.";
              }
              ...
              core.error("Error while reading or parsing the JSON: " + err.message)
              core.setFailed(err)
            }
```
