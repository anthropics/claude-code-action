...
- **Token-free Git authentication**: Set `git_credential_helper: true` to keep the GitHub token out of the repository's `.git/config`. The action removes any checkout header and configures a credential helper that reads the short-lived token at authentication time. This option is independent of `allowed_non_write_users` and is useful whenever workspace code is untrusted. Keep workflow permissions minimal and do not use a personal access token.
