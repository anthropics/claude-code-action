...
| `enable_all_project_mcp_servers` | Auto-enable every MCP server in the checkout's `.mcp.json`. Defaults to `true` (except under `workflow_run`, where it defaults to `false`); in PR contexts the action restores project config from the base ref before launching.      | No       | ""            |
