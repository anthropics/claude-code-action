...
| `enable_all_project_mcp_servers` | Auto-enable every MCP server in the checkout's `.mcp.json`. Automatically enabled when project config is restored from the base ref; otherwise off.                                                                                    | No       | `false`       |
