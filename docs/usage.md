...
| `include_comments_by_actor`      | Comma-separated list of actor usernames to INCLUDE in comments. Entries are exact, case-sensitive logins; the only wildcard is the literal `*[bot]`, matching all bots. Empty (default) includes all actors                            | No       | ""                          |
| `exclude_comments_by_actor`      | Comma-separated list of actor usernames to EXCLUDE from comments. Entries are exact, case-sensitive logins; the only wildcard is the literal `*[bot]`, matching all bots. If an actor matches both lists, exclusion takes priority     | No       | ""                          |
