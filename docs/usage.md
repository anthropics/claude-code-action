    ...
    steps:
      - uses: anthropics/claude-code-action@v1
        with:
          ...
          # (pin to a branch, tag, or commit SHA with '#'; unpinned URLs track the default branch)
          # plugin_marketplaces: "https://github.com/user/marketplace1.git#v1.2.3\nhttps://github.com/user/marketplace2.git"
...
| `plugin_marketplaces`            | Newline-separated list of Claude Code plugin marketplace Git URLs to install from (e.g., see example in workflow above). Each URL may be pinned to a branch, tag, or commit SHA with a `#` suffix (e.g., `...marketplace.git#v1.2.3`); unpinned URLs track the default branch. Marketplaces are added before plugin installation | No       | ""                          |
