...
| `git_credential_helper`          | Use a token-free credential helper so the GitHub token is not stored in `.git/config`. Independent of `allowed_non_write_users`; see [Security](./security.md)                                                                         | No       | `false`                     |
