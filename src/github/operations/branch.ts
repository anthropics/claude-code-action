 ...
 *
 * Valid branch names:
 * - Start with a Unicode letter or number, underscore, or @ (not dash, to prevent option injection)
 * - Contain only Unicode letters/numbers/combining marks, forward slash, hyphen, underscore, period, hash (#), plus (+), comma (,), at sign (@), or parentheses
...
export function validateBranchName(branchName: string): void {
  ...
  // Unicode letters and numbers support non-ASCII branch names; combining marks
  // preserve decomposed characters (e.g. accented letters and Japanese dakuten).
  ...
  const validPattern = /^[\p{L}\p{N}@_][\p{L}\p{N}\p{M}/_.#+,@()-]*$/u;

  if (!validPattern.test(branchName)) {
    throw new Error(
      `Invalid branch name: "${branchName}". Branch names must start with a Unicode letter or number, underscore, or '@' and contain only Unicode letters, numbers, combining marks, forward slashes, hyphens, underscores, periods, hashes (#), plus signs (+), commas (,), at signs (@), or parentheses.`,
    );
  }
