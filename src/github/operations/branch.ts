 ...
 * - Contain only alphanumeric, forward slash, hyphen, underscore, period, hash (#), plus (+), comma (,), parentheses ( ), or at sign (@)
...
export function validateBranchName(branchName: string): void {
  ...
  // Strict whitelist pattern: alphanumeric or @ start, then alphanumeric/slash/hyphen/underscore/period/hash/plus/comma/parentheses/at-sign.
  ...
  // ( ) are valid per git-check-ref-format and commonly appear in branch names derived from
  //   conventional-commit scope syntax (e.g. "feat(example)-valid-branch") or package scopes
  //   (e.g. "chore(deps)/...").
  ...
  if (!validPattern.test(branchName)) {
    throw new Error(
      `Invalid branch name: "${branchName}". Branch names must start with an alphanumeric character, underscore, or '@' and contain only alphanumeric characters, forward slashes, hyphens, underscores, periods, hashes (#), plus signs (+), commas (,), parentheses ( ), or at signs (@).`,
    );
  }
